...
<template>
  ...
  >
    ...
    <div class="absolute inset-0 pointer-events-none overflow-hidden">
      <!-- Central warm glow behind image -->
      <div
        class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] rounded-full opacity-[0.05]"
        style="background: radial-gradient(ellipse at center, rgba(216,168,79,0.15) 0%, rgba(216,168,79,0.04) 40%, transparent 70%)"
      />
      <!-- Subtle grid -->
      <div
        class="absolute inset-0 opacity-[0.02]"
        style="background-image: linear-gradient(rgba(216,168,79,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(216,168,79,0.4) 1px, transparent 1px); background-size: 80px 80px"
      />
      <!-- Floating particles -->
      <div class="absolute top-[15%] left-[10%] w-1 h-1 rounded-full" style="background: rgba(216,168,79,0.3); animation: float 8s ease-in-out infinite" />
      <div class="absolute top-[25%] right-[15%] w-1.5 h-1.5 rounded-full" style="background: rgba(99,102,241,0.25); animation: float 10s ease-in-out infinite 2s" />
      <div class="absolute bottom-[30%] left-[20%] w-1 h-1 rounded-full" style="background: rgba(216,168,79,0.2); animation: float 9s ease-in-out infinite 4s" />
      <div class="absolute bottom-[20%] right-[10%] w-1.5 h-1.5 rounded-full" style="background: rgba(16,185,129,0.2); animation: float 11s ease-in-out infinite 1s" />
    </div>
    ...
    <div class="relative z-10 max-w-6xl mx-auto">
      ...
      >
        <span
          ...
          style="border: 1px solid rgba(216,168,79,0.2); background: rgba(216,168,79,0.05); color: #D8A84F"
        >
          <span class="w-1.5 h-1.5 rounded-full animate-pulse" style="background: #D8A84F" />
          {{ t('aiIntro.label') }}
        </span>
      </div>
      ...
      >
        <div class="relative max-w-4xl mx-auto">
          ...
          <div
            ...
            "
          >
            <!-- Top chrome bar (like a window title bar) -->
            <div
      ...
      >
        <div
          ...
          @mouseenter="($event.currentTarget as HTMLElement).style.borderColor = 'rgba(216,168,79,0.2)'"
          @mouseleave="($event.currentTarget as HTMLElement).style.borderColor = 'rgba(255,255,255,0.05)'"
        >
          <div class="w-8 h-8 rounded-lg flex items-center justify-center mx-auto mb-3 transition-colors duration-300" style="background: rgba(216,168,79,0.08)">
            <span class="text-sm font-bold" style="color: #D8A84F; font-family: 'Space Grotesk', sans-serif">{{ index + 1 }}</span>
          </div>
          <h3
            class="font-semibold text-sm mb-1.5"
            style="font-family: 'Space Grotesk', sans-serif; color: #F5F1E8"
          >
            {{ t(`aiIntro.feature${index + 1}.title`) }}
          </h3>
          <p class="text-xs leading-relaxed" style="color: #6B6560">
            {{ t(`aiIntro.feature${index + 1}.desc`) }}
          </p>
        </div>
      </div>
    </div>
